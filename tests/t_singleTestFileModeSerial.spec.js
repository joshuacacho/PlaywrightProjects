...
//this will run the tests in this file in a serial manner instead of parallel which is the default, this is useful when you have tests that are dependent on each other or when you want to run tests in a specific order, it ensures that the tests are executed one after the other rather than concurrently, which can help to avoid issues related to shared state or resources between tests.
...
test("GetByLabel & Playwright UI Runner", async ({ page }) => {
  ...
  //playwright GetByLabel checkbox 
    //FAILED INTENTIONALLY to test test.describe.configure({mode: 'serial'});  where one test is depending on another one passing
  const getByLabelCheckCox = page.getByLabel(
    "Check me out if you Love IceCreams!daffadsfd",
  );
