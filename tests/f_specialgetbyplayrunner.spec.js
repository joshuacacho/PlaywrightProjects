...
async function pickDate(page, yearToGoTo, monthToGoTo, dayToGoTo) {
  ...
  try {
    ...
    //grab current year
    const currentYear = await page.locator(".react-calendar__navigation__label span").textContent();

    //select the small forward year icon until we get to the yearToGoTo value
    const smallArrowForward = page.locator(".react-calendar__navigation__next-button");
    ...
    console.log(finalDate, `${yearToGoTo}-${indexedMonthToGoTo}-${dayToGoWith0orNot}`);

    //await page.pause();

    //asserting final
    expect(finalDate).toEqual(`${yearToGoTo}-${indexedMonthToGoTo}-${dayToGoWith0orNot}`);
    ...
    console.error(
      `❌ Failed to set date ${error.stack}`,
    );
    throw error;
  }
}
