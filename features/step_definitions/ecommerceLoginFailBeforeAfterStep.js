...
Given("Before After Step Hooks - the user navigates to the login page within {string}", { timeout: 10 * 1000 }, async function (loginPageURL) { 
    await this.page.goto(loginPageURL); // ✅ page already set up by Before hook
});

When("Before After Step Hooks - the user enters valid username {string} and the password {string}", async function (username, password) {
...
When("Before After Step Hooks - attempts to click the login button", async function () {
    await this.page.click('#login'); // ✅ same page shared across steps
})

Then("Before After Step Hooks - the user should be logged in successfully with a URL containing {string}", async function (loginURLFragment) {
