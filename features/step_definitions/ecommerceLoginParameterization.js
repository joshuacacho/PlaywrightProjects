...
Given("Parameterized the user navigates to the login page within {string}", { timeout: 10 * 1000 }, async function (loginPageURL) { 
    await this.page.goto(loginPageURL); // ✅ page already set up by Before hook
});

When("Parameterized the user enters valid username {string} and the password {string}", async function (username, password) {
...
When("Parameterized attempts to click the login button", async function () {
